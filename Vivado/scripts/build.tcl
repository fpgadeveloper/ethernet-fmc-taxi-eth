...
# Possible targets
# UPDATER START
dict set target_dict kcu105 { xilinx.com kcu105 mb-us { 0 1 2 3 } }
dict set target_dict zcu104 { xilinx.com zcu104 zynqmp { 0 1 2 3 } }
# UPDATER END
...
# ---------------------------------------------------------------------------
# HDL sources: the Taxi transport library (git submodule, CERN-OHL-S-2.0) and
# the MIT-licensed wrapper that the block design instantiates as a module
# reference (src/hdl/taxi_rgmii_mac.sv). See submodules/README.md.
# ---------------------------------------------------------------------------
set taxi_dir [file normalize "$origin_dir/../submodules/taxi"]
...
source [file normalize "$origin_dir/scripts/taxi_sources.tcl"]
add_files -norecurse -fileset sources_1 $taxi_rtl
add_files -norecurse -fileset sources_1 [glob [file normalize "$origin_dir/src/hdl/*.sv"]]
set_property file_type SystemVerilog [get_files -of_objects [get_filesets sources_1] *.sv]
# The block-design module reference needs a Verilog (not SystemVerilog) top file
add_files -norecurse -fileset sources_1 [glob [file normalize "$origin_dir/src/hdl/*.v"]]
# Taxi's Tcl timing constraints locate cells by ORIG_REF_NAME: implementation only
foreach f $taxi_xdc {
  add_files -norecurse -fileset constrs_1 $f
  set_property used_in_synthesis false [get_files -of_objects [get_filesets constrs_1] $f]
}
...
# The MicroBlaze/DDR4 targets need a post-route hold-fixing pass. The AXI
# SmartConnect crosses between the memory controller's two user clocks
# (100 MHz addn_ui_clkout1 and 300 MHz c0_ddr4_ui_clk) with its related-clocks
# logic, whose hold checks are at zero edge separation: after the MMCM phase
# error and the clock-tree pessimism there is barely 0.2 ns of budget for the
# data path, and whether route_design alone can stretch every one of those few
# hundred paths depends on how congested that corner of the die ends up.
if { $bd_script eq "mb-us" } {
